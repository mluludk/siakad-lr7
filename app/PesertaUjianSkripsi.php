<?php
	...
	class PesertaUjianSkripsi extends Model
	{
		...
		public function scopeJadwalUjian($query, $mahasiswa_id, $jenis)
		{
			...
			-> where('jenis', $jenis)
			-> select(
			'jadwal_ujian_skripsi.nama as nama_ujian', 'no_surat', 
			'jadwal_ujian_skripsi_gelombang.nama as nama_gelombang',
			'ruang.nama as ruang',
			'tanggal', 'jam_mulai', 'jam_selesai',
			'd1.gelar_depan as p_gd', 'd1.nama as p_nama', 'd1.gelar_belakang as p_gb',
			'd2.gelar_depan as k_gd', 'd2.nama as k_nama', 'd2.gelar_belakang as k_gb',
			'd3.gelar_depan as k_gd', 'd3.nama as s_nama', 'd3.gelar_belakang as s_gb'
			);
		}
