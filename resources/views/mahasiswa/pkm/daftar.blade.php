...
<section class="content-header">
	...
	<ol class="breadcrumb">
		...
		<li><a href="{{ route('mahasiswa.pkm.lokasi.peserta.index', $pkm -> id) }}"> Peserta PKM</a></li>
	...
	<div class="box-body">
		<div class="row">
			...
			<div class="col-sm-9">
				...
				{!! Form::hidden('matkul_id', $pkm -> matkul_id) !!}
				
				<div class="form-group">
					<label class="col-sm-2 control-label">Periode PKM :</label>
					<div class="col-sm-9">
						<p class="form-control-static">
							<strong>{{ $pkm -> tapel -> nama }} ({{ formatTanggal(date('Y-m-d', strtotime($pkm -> tanggal_mulai))) }} - {{ formatTanggal(date('Y-m-d', strtotime($pkm -> tanggal_selesai))) }})</strong>
						</p>
					</div>
				</div>
				
				<div class="form-group">
					<label class="col-sm-2 control-label">Tempat PKM :</label>
					<div class="col-sm-10">
						@if($show)
						<p class="form-control-static">{{ $lokasi }}</p>
						@else
						{!! Form::select('pkm_lokasi_id', $lokasi, null, array('class' => 'form-control', 'data-placeholder' => 'Pilih Lokasi')) !!}
						@endif
					</div>
				</div>
				...
				<div class="form-group">
					<div class="col-sm-offset-2 col-sm-10">
						@if($show)
						<a href="{{ route('mahasiswa.pkm.daftar.cetak', [$lokasi_pkm_mhs -> pkm_lokasi_id, $data -> id]) }}" class="btn btn-info btn-flat" ><i class="fa fa-print"></i> Cetak</a>				
						@else		
