...
<div class="row">
	...
	<div class="col-sm-9">
		...
		<div class="box box-danger">
			<div class="box-header with-border">
				<h3 class="box-title">Status Pendaftaran Wisuda</h3>
			</div>
			<div class="box-body">
				<table class="table table-bordered table-striped">
					...
					<tbody>
						@if(!$status)
						<tr>
							<td colspan="5" align="center"><span class="text-danger"><strong>BELUM TERDAFTAR</strong></span></td>
						</tr>
						@else
						<tr>
							<td>{{ $status -> nama ?? '-' }}</td>
							<td>{{ $status -> tanggal ?? '' }}</td>
							<td>{{ $status -> SKYudisium ?? '-' }}</td>
							<td>{{ $status -> tglSKYudisium ?? '-' }}</td>
							<td>
